...
exports.smsg = (xizone, m, store) => {
  ...
  if (m.key) {
    ...
    if (m.isGroup) m.participant = xizone.decodeJid(m.key.participant) || '';
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.delete = () => xizone.sendMessage(m.quoted.chat, {
        delete: vM.key
      });
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => xizone.copyNForward(jid, vM, forceForward, options);
      m.quoted.download = () => xizone.downloadMediaMessage(m.quoted);
    }
  }
  if (m.msg.url) m.download = () => xizone.downloadMediaMessage(m.msg);
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? xizone.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : xizone.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => xizone.copyNForward(jid, m, forceForward, options);
  xizone.appenTextMessage = async (text, chatUpdate) => {
